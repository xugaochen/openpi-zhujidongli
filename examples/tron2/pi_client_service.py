...
class ServiceConfig:
    ...
    static_rounds_required: int
    min_rounds_before_static_finish: int
...
class VLAWebSocketService:
    ...
    def _execute_task_sync(self, task: str) -> None:
        """Run one task to completion, stop, timeout, or error."""
        static_rounds = 0
        previous_last_action: Optional[np.ndarray] = None
        ...
        try:
            ...
            for infer_round in range(1, self.config.max_infer_rounds + 1):
                ...
                if self._static_finish_reached(
                    actions=actions,
                    previous_last_action=previous_last_action,
                    infer_round=infer_round,
                    current_static_rounds=static_rounds,
                ):
                    static_rounds += 1
                else:
                    static_rounds = 0

                previous_last_action = actions[-1].copy()

                # Prefer an explicit completion signal when the policy server
                # provides one. This is more reliable than a fixed action count.
                ...
                if (
                    ...
                    and infer_round >= self.config.min_rounds_before_static_finish
                    and static_rounds >= self.config.static_rounds_required
                ):
                    logging.info(
                        "task=%s reached near-static finish condition after %d rounds",
                        task,
                        infer_round,
                    )
                    self._finish_done(task)
    ...
    def _static_finish_reached(
        self,
        *,
        actions: np.ndarray,
        previous_last_action: Optional[np.ndarray],
        infer_round: int,
        current_static_rounds: int,
    ) -> bool:
        """Optional heuristic for models without an explicit EOS signal.

        This is intentionally disabled by default. A near-static action plan is
        not a semantic success signal; enable it only after validating it on your
        own task trajectories.
        """
        del infer_round, current_static_rounds  # documented inputs for clarity

        if not self.config.static_finish_enabled:
            return False

        deltas: list[np.ndarray] = []
        if len(actions) > 1:
            deltas.append(np.diff(actions, axis=0))
        if previous_last_action is not None:
            deltas.append(actions[:1] - previous_last_action[None, :])

        if not deltas:
            return False

        max_delta = max(float(np.max(np.abs(delta))) for delta in deltas)
        return max_delta <= self.config.static_action_threshold

    def _finish_done(self, task: str) -> None:
        with self._state_lock:
            self._running = False
            self._active_task = None
...
def parse_args() -> ServiceConfig:
    ...
    # Optional heuristic for models that have no explicit task-completion token.
    parser.add_argument("--enable-static-finish", action="store_true")
    parser.add_argument("--static-action-threshold", type=float, default=0.003)
    parser.add_argument("--static-rounds-required", type=int, default=2)
    parser.add_argument("--min-rounds-before-static-finish", type=int, default=3)
    parser.add_argument(
    ...
    if args.static_rounds_required <= 0:
        parser.error("--static-rounds-required must be > 0")
    ...
    return ServiceConfig(
        ...
        static_finish_enabled=args.enable_static_finish,
        static_action_threshold=args.static_action_threshold,
        static_rounds_required=args.static_rounds_required,
        min_rounds_before_static_finish=args.min_rounds_before_static_finish,
